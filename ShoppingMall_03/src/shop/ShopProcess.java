...
public class ShopProcess {
	...
	public void insertAdminUser() { // 관리자 계정 생성
		...
		// 관리자 아이디는 admin. 비밀번호는 1111, 이름은 관리사
	...
	public void insertUserInUserList() { // 유저등록
		...
		while(isFind) {
			...
			for(int i=0; i<ShopRepository.lastUserNumber; i++){
				// 입력받은 아이디와 같은 아이디가 없다면
				if(!id.equals(ShopRepository.users.get(i).getId())){
					...
					System.out.println("해당아이디를 사용하는 회원이 있습니다. 다른 아이디를 입력해주세요.");
					break;
	...
	public void login() { // 로그인
		...
		while(!firstIsFind) {
			...
			for(int i=0; i<ShopRepository.users.size(); i++){
				...
				if(ShopRepository.users.get(0).getPw().equals(pw)) {
				...
				}else if(ShopRepository.users.get(i).getPw().equals(pw)) {

					System.out.println("환영합니다! " + ShopRepository.users.get(i).getId() + "님");
					secondIsFind = true;
					ShopRepository.loginUserNumber = i; // 로그인한 유저의 번호를 저장
					mainMenu();	// 유저 메인 페이지로 이동
					break;
				}
			}
	...
	public void displayCartList() { // 카트리스트 출력
		...
		if(sum == 0){
			System.out.println("아무 상품이 없습니다. 메인메뉴로 돌아갑니다");
			mainMenu();
		}
		...
		while(true) {
			...
			if(selectedMenu == 'y') {
				...
				if(ShopRepository.carts.size()!=0) { // 카트가 비어있지 않다면
					...
					System.out.println("카트리스트에 상품이 없습니다. 메인메뉴로 돌아갑니다.");
					mainMenu();
